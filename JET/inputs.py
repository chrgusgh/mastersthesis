...
T0            = T0[T0r_pos]
T0            = T0[:-9]
Ip0           = np.abs(p0['Ip_A'][:]) # positive current
assimilation  = 1e-1 # How much of the injected materials enters the tokamak chamber
...
j_par_at_Bmin = np.abs(eqdsk.get_Jpar_at_Bmin(psi_n))
j_par_at_Bmin = j_par_at_Bmin[:-19]
r             = eqdsk.get_r(psi_n)
r             = r[:-19]
...
T_f           = 100  # eV
t_TQ          = 1.75e-4 # s, assumption based off of paper by Insulander Björk
tau_TQ        = utils.calculate_tau_TQ(t_TQ, T_i, T_f)
v_th          = utils.calculate_v_th(T_f, m_e_eV)
#dBB_cold      = utils.calculate_dBB(a, R0, t_TQ, v_th)
#dBB_re        = dBB_cold #utils.calculate_dBB(a, R0, T_f, t_TQ, const.c)
dBB_cold = 1e-4
dBB_re = dBB_cold
